...
const buttonOptions = [
    { value: '', label: __( '— Select a payment button —', 'opay-payment-gateway' ) },
    ...buttons,
];
...
export default function Edit( { attributes, setAttributes } ) {
    ...
    return (
        <>
            <InspectorControls>
                <PanelBody title={ __( 'Button Settings', 'opay-payment-gateway' ) } initialOpen={ true }>
                    <SelectControl
                        label={ __( 'Payment Button', 'opay-payment-gateway' ) }
                        value={ buttonId }
                        options={ buttonOptions }
                        onChange={ ( value ) => setAttributes( { buttonId: value } ) }
                        help={ __( 'Choose an existing Opay payment button.', 'opay-payment-gateway' ) }
                    />
                    <TextControl
                ...
                { ! buttonId ? (
                    ...
                    <div className="opay-block-preview">
                        ...
                        <p className="opay-block-meta">
                            { __( 'Mode:', 'opay-payment-gateway' ) } <strong>{ environment }</strong>
                            &nbsp;|&nbsp;
