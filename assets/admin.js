...
( function ( $ ) {
    ...
    $( '.opay-keys-form' ).on( 'submit', function ( e ) {
        ...
        ajaxPost( 'opay_save_keys', { environment: env, pk, sk: skToSend }, function ( err, res ) {
            if ( err ) {
                showNotice( err, 'error' );
            } else {
    ...
    function renderApiKeyInfo( data ) {
        ...
        if ( ! keys.length ) {
            $info.html( '<p>No API keys found. Generate keys from the Opay dashboard.</p>' );
            return;
        }

        let html = '<table class="widefat fixed opay-mini-table"><thead><tr><th>Type</th><th>Mode</th><th>Key (masked)</th></tr></thead><tbody>';
        keys.forEach( ( k ) => {
    ...
    function loadTransactions( page ) {
        ...
        ajaxPost( 'opay_load_transactions', data, function ( err, res ) {
            ...
            rows.forEach( ( tx ) => {
                ...
                const statusClass = tx.status === 'succeeded' ? 'opay-status-success'
                    : tx.status === 'failed' ? 'opay-status-failed'
                    : 'opay-status-pending';
    ...
    function loadButtons() {
        ...
        ajaxPost( 'opay_load_buttons', {}, function ( err, res ) {
            ...
            if ( ! buttons.length ) {
                $body.html( '<tr><td colspan="6">No payment buttons found. Create one above.</td></tr>' );
                return;
            }
